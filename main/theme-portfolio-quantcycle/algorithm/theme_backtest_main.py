...
def portfolio_formation(portfolio_name, number_assets, symbol_industry, weighting_approach,
    ...
    if portfolio_name in ['US_Profit_B','US_Profit_S','US_Safety_B','US_Safety_S',
                           'US_Tech_B','US_Tech_S']:
        if 'GLIBA' in symbol_list:
            symbol_list.remove('GLIBA')
    investment_univ = list(set(investment_univ) & set(factors.index.tolist()) & set(symbol_list))
    # list = ['AAPL', 'MSFT', 'AMZN', 'FB', 'GOOG', 'TSLA', 'NVDA', 'BRK.B', 'JPM', 'DIS', 'NFLX', 'PYPL', 'T']
    # investment_univ.extend(list)
    ...
    if date ==datetime(2020,12,31):
        print("价格过高被筛掉",last_period_factors[last_period_factors['LOTPRICE'] > cash * upper_bound]['LOTPRICE'])
    ...
    if portfolio_name in ['US_Tech_B', 'US_Tech_S']:
        symbol_industry=symbol_industry[symbol_industry['GICSECTOR']==45]
    ...
    if portfolio_name in ['HK_Hstech_B','HK_Hstech_S']:
        ...
        for ind in ind_list:
            ...
            for col in list(criteria.keys()):
                ...
                temp['ind_num']=len(temp)
                temp[col+'rank_pct in industry']=temp[col + '_rank']/temp['ind_num']

            temp['ind_rank']=temp['ind_zscore'].rank(ascending=True)
            temp = temp[temp['ind_rank'] <= num_top_ind]
    ...
    if portfolio_name in ['HK_Hstech_B','HK_Hstech_S']:
        ...
        rankings_main = rankings[(rankings['ind_rank'] <= 3)]
