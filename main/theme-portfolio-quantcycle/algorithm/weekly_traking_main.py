...
if __name__ == "__main__":
    portfolio_name_list = ['CN_Quality','CN_Value','HK_Hstech_B','HK_Hstech_S',
                           ...
                           'US_Tech_B','US_Tech_S','US_5G']
    portfolio_name_list = ['US_5G']
    task='tracking'
    ...
    # #                        'US_Tech_B','US_Tech_S']
    # task='tracking'
    # update_tracking_file = os.path.join(addpath.result_path, task, 'strategy_kit','Theme_portfolio.xlsx')
    # writer = pd.ExcelWriter(update_tracking_file, engine="xlsxwriter")
    #
    ...
    #     print(portfolio_name)
    #
    #     update_tracking_file_in=os.path.join(addpath.result_path,task,'strategy_kit',portfolio_name+'.xlsx')
    #     reader=pd.ExcelWriter(update_tracking_file_in,engine="xlsxwriter")
    #     last_reb_weight_in=pd.read_excel(reader,sheet_name='weighting',index_col=0)
    #     pv_in=pd.read_excel(reader,sheet_name='pv',index_col=0)[[100]]
    #     last_reb_weight_in.columns=[portfolio_name]
    #     pv_in.columns=[portfolio_name]
    #
    #     weight_all = pd.concat([weight_all,last_reb_weight_in], axis=0)
    #     pv_all = pd.concat([pv_all,pv_in], axis=1)
    # print(weight_all)
    # print(pv_all)
    # weight_all.to_excel(writer,sheet_name='weighting')
