...
class PortfolioRecord:
    def __init__(self, universe, pv0=None, simple_pv=None, position_list=None, pv_list=None, timestamp_list=None,
                 ...
                 signal_remark=None, trade_record=None,cost_list = None, record_class='fx'):
        self.strategy_id = strategy_id  # strategy_id for the universe and parameter -int -example: 14
        ...
        self.timestamp_list = timestamp_list  # timestamp of every sample (list) --example: [1765165165.0,1765165168.0,]
        self.position_list = position_list  # position of every sample (list_list) --example: [[100,400,],[100,300,],]
        self.pv_list = pv_list  # pv of every sample (list_list) --example: [[4038,4038,],[4038,4038,],]
        self.ref_aum_list = ref_aum_list
        self.ccy_name_list = ccy_name_list
        self.ccy_metrics_list = ccy_metrics_list
        self.sample_len = 0
        if position_list is not None:
            self.sample_len = len(position_list)  # number of sample (int) --example: 145
    ...
    def array_to_list(self):
        ...
        self.ccy_metrics_list = [array.tolist() for array in self.ccy_metrics_list]
