...
def investment_univ_helper(stock_list, formation_date, cn_path, trading_data_total):
	...
	investment_univ.to_csv(os.path.join(cn_path, "investment_univ", formation_date.strftime('%Y-%m-%d') + ".csv"),
						   index=False)
...
if __name__ == "__main__":
	...
	trading_path=os.path.join(cn_path, "trading")
	...
	for ticker in csv_name_list:
		ticker=ticker[:-4]
		# print(ticker)
		data_in=pd.read_csv(os.path.join(cn_path, "trading", ticker+'.csv'), parse_dates=[0], index_col=0)
		stock_list.append(ticker)
		trading_data_total[ticker] = pd.read_csv(os.path.join(cn_path, "trading", ticker+'.csv'), parse_dates=[0], index_col=0)

	with open(os.path.join(cn_path, "trading_date_dict.pickle"), "wb") as fp:  # Pickling
		pickle.dump(trading_data_total, fp, protocol=pickle.HIGHEST_PROTOCOL)
	with open(os.path.join(cn_path, "trading_date_dict.pickle"), "rb") as fp:  # Pickling
	...
	for i in range(750, len(trading_data_total_keys), 1):
		print(i)
	...
	for formation_date in formation_dates:
		...
		investment_univ_helper(stock_list, formation_date, cn_path, trading_data_total)
	pool.close()
	pool.join()
