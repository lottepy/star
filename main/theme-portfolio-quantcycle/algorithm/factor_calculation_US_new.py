...
def cal_factors(symbol, rebalance_freq):
	...
	us_fin_path = os.path.join(us_path, "Quarterly Financial csv")
	us_tra_path = os.path.join(us_path, "trading")

	# Mkt factor
	mkt_dt = pd.read_csv(os.path.join(us_path,'reference', "market_index.csv"), parse_dates=[0], index_col=0)
	...
	result.to_csv(os.path.join(us_path, "Factors_new", symbol + ".csv"))
	print(os.path.join(us_path, "Factors_new", symbol + ".csv")+'writen')
	# return result
...
if __name__ == "__main__":
	...
	factors_path = os.path.join(us_path, "Factors_new")
	CS_factors_path = os.path.join(us_path, "CS_Factors_new")

	factors_files = os.listdir(factors_path)
	factors = {}
	for symbol in factors_files:
		print(symbol)
		temp=pd.read_csv(os.path.join(factors_path, symbol),index_col=0)
		factors[symbol]=temp

	for iu_file in iu_files:
		print(iu_file)

		datain = os.path.join(iu_path, iu_file)
		iu_list = pd.read_csv(datain)['symbol'].tolist()
		...
		for symbol in iu_list:
			...
			if file_name in list(factors.keys()):
				temp_symbol = factors[file_name][factors[file_name].index == iu_file[:-4]]
				dataout.append(temp_symbol)
			else:
				print(symbol + 'in investment universe is not in factor list!')
		dataout = pd.concat(dataout)
		dataout.to_csv(os.path.join(CS_factors_path,iu_file),index=False)
	...
	# 	factors[symbol]=pd.read_csv(os.path.join(factors_path, factors_file),index_col=0)
	# 	factors[symbol]['PRE_BS_TOT_ASSET']=factors[symbol]['BS_TOT_ASSET'].shift(12)
	#
	# 	factors[symbol]['date']=factors[symbol].index
	...
	#
	# cpi_data=pd.read_csv(os.path.join(us_path,'US_CPI.csv'))
	#
	# for iu_file in iu_files:
	# 	print(iu_file)
	# 	date =str(iu_file[0:7]+'-01')
	# 	cpi=cpi_data[cpi_data['date']==date].iloc[0,1]
	# 	tmp = pd.read_csv(os.path.join(iu_path, iu_file))
	# 	tmp_list = tmp['symbol'].tolist()
	# 	dataout=[]
	# 	# date=datetime(int(iu_file[0:4]),int(iu_file[5:7]),int(iu_file[8:10]))
	# 	date=iu_file[0:4]+'-'+iu_file[5:7]+'-'+iu_file[8:10]
	#
	# 	inve_list = []
	...
	# 			# print(symbol)
	# 			temp_symbol=factors[symbol][factors[symbol]['date']==date]
	# 			dataout.append(temp_symbol)
	# 			inve_list.append(symbol)
	# 	inve_list=pd.DataFrame(inve_list,columns=['symbol'])
	...
	# 	CSfactors_path = os.path.join(us_path, "CS_Factors_new")
	...
	# CSfactors_path = os.path.join(us_path, "CS_Factors_new")
	# read_path=CSfactors_path
	# CSfactors_files = os.listdir(read_path)
	# cpi_data=pd.read_csv(os.path.join(us_path,'US_CPI.csv'))
	# for CSfactors_file in CSfactors_files:
	# 	date =str(CSfactors_file[0:7]+'-01')
	# 	cpi=cpi_data[cpi_data['date']==date].iloc[0,1]
    #
	# 	data=pd.read_csv(os.path.join(CSfactors_path,CSfactors_file))
        #
		# data['PRE_ASSET']=data['NET_INCOME']*2/data['ROA']-data['BS_TOT_ASSET']
		# data['PRE_ASSET_SEMI']=data['NET_INCOME_SEMI']*2/data['ROA_SEMI']-data['BS_TOT_ASSET']
		# data['PRE_ASSET_TTM']=data['NET_INCOME_TTM']*2/data['ROA_TTM']-data['BS_TOT_ASSET']
		# data['PRE_EQUITY']=data['NET_INCOME']*2/data['ROE']-data['TOT_EQUITY']
		# data['PRE_EQUITY_SEMI']=data['NET_INCOME_SEMI']*2/data['ROE_SEMI']-data['TOT_EQUITY']
		# data['PRE_EQUITY_TTM']=data['NET_INCOME_TTM']*2/data['ROE_TTM']-data['TOT_EQUITY']
        #
		# data['PRE_NET_INCOME']=data['NET_INCOME']/(data['EARNGROWTH']+1)
		# data['PRE_NET_INCOME_SEMI']=data['NET_INCOME_SEMI']/(data['EARNGROWTH_SEMI']+1)
		# data['PRE_NET_INCOME_TTM']=data['NET_INCOME_TTM']/(data['EARNGROWTH_TTM']+1)
		# data['PRE_REVENUE']=data['SALES_REV_TURN']/(data['REVENUEGROWTH']+1)
		# data['PRE_REVENUE_SEMI']=data['SALES_REV_TURN_SEMI']/(data['REVENUEGROWTH_SEMI']+1)
		# data['PRE_REVENUE_TTM']=data['SALES_REV_TURN_TTM']/(data['REVENUEGROWTH_TTM']+1)
		# data['PRE_OPCF']=data['OPCFOA']/(data['OPCFOAGROWTH']+1)*data['PRE_EQUITY']
		# data['PRE_OPCF_SEMI']=data['OPCFOA_SEMI']/(data['OPCFOAGROWTH_SEMI']+1)*data['PRE_EQUITY_SEMI']
		# data['PRE_OPCF_TTM']=data['OPCFOA_TTM']/(data['OPCFOAGROWTH_TTM']+1)*data['PRE_EQUITY_TTM']
        #
		# data['PRE_GROSS_PROFIT']=data['GROSS_PROFIT_MARGIN']/(data['GPOAGROWTH']+1)*data['PRE_REVENUE']
		# data['PRE_GROSS_PROFIT_SEMI']=data['GROSS_PROFIT_MARGIN_SEMI']/(data['GPOAGROWTH_SEMI']+1)*data['PRE_REVENUE_SEMI']
		# data['PRE_GROSS_PROFIT_TTM']=data['GROSS_PROFIT_MARGIN_TTM']/(data['GPOAGROWTH_TTM']+1)*data['PRE_REVENUE_TTM']
        #
		# data['ROAGROWTH']=(data['NET_INCOME']-data['PRE_NET_INCOME'])/data['PRE_ASSET']
		# data['ROAGROWTH_SEMI']=(data['NET_INCOME_SEMI']-data['PRE_NET_INCOME_SEMI'])/data['PRE_ASSET_SEMI']
		# data['ROAGROWTH_TTM']=(data['NET_INCOME_TTM']-data['PRE_NET_INCOME_TTM'])/data['PRE_ASSET_TTM']
		# data['ROEGROWTH']=(data['NET_INCOME']-data['PRE_NET_INCOME'])/data['PRE_EQUITY']
		# data['ROEGROWTH_SEMI']=(data['NET_INCOME_SEMI']-data['PRE_NET_INCOME_SEMI'])/data['PRE_EQUITY_SEMI']
		# data['ROEGROWTH_TM']=(data['NET_INCOME_TTM']-data['PRE_NET_INCOME_TTM'])/data['PRE_EQUITY_TTM']
		# data['OPCFOAGROWTH']=(data['OPCF']-data['PRE_OPCF'])/data['PRE_ASSET']
		# data['OPCFOAGROWTH_SEMI']=(data['OPCF_SEMI']-data['PRE_OPCF_SEMI'])/data['PRE_ASSET_SEMI']
		# data['OPCFOAGROWTH_TTM']=(data['OPCF_TTM']-data['PRE_OPCF_TTM'])/data['PRE_ASSET_TTM']
		# data['OPCFOEGROWTH']=(data['OPCF']-data['PRE_OPCF'])/data['PRE_EQUITY']
		# data['OPCFOEGROWTH_SEMI']=(data['OPCF_SEMI']-data['PRE_OPCF_SEMI'])/data['PRE_EQUITY_SEMI']
		# data['OPCFOEGROWTH_TTM']=(data['OPCF_TTM']-data['PRE_OPCF_TTM'])/data['PRE_EQUITY_TTM']
        #
		# data['GPOAGROWTH']=(data['GROSS_PROFIT']-data['PRE_GROSS_PROFIT'])/data['PRE_ASSET']
		# data['GPOAGROWTH_SEMI']=(data['GROSS_PROFIT_SEMI']-data['PRE_GROSS_PROFIT_SEMI'])/data['PRE_ASSET_SEMI']
		# data['GPOAGROWTH_TTM']=(data['GROSS_PROFIT_TTM']-data['PRE_GROSS_PROFIT_TTM'])/data['PRE_ASSET_TTM']
		# data['GPOEGROWTH']=(data['GROSS_PROFIT']-data['PRE_GROSS_PROFIT'])/data['PRE_EQUITY']
		# data['GPOEGROWTH_SEMI']=(data['GROSS_PROFIT_SEMI']-data['PRE_GROSS_PROFIT_SEMI'])/data['PRE_EQUITY_SEMI']
		# data['GPOEGROWTH_TTM']=(data['GROSS_PROFIT_TTM']-data['PRE_GROSS_PROFIT_TTM'])/data['PRE_EQUITY_TTM']
		...
		# temp=temp.drop(index=temp.index[temp<=0])
		# temp=temp.apply(math.log)
		# data['O_SCORE']=-1.32-0.407*temp+6.03*data['BS_TOT_LIAB2']/data['BS_TOT_ASSET']-1.43*data['WORKING_CAPITAL']/data['BS_TOT_ASSET']+0.0757*data['CURRLIAB']/data['CURRASSET']-1.72*data['TL>TA']-2.37*data['ROA_TTM']-1.83*data['OPCF']/data['BS_TOT_LIAB2']+0.285*data['loss_for_2y']-0.521*(data['NET_INCOME_TTM']-data['PRE_NET_INCOME_TTM'])/(abs(data['NET_INCOME_TTM'])+abs(data['PRE_NET_INCOME_TTM']))
        #
		# data.to_csv(os.path.join(CSfactors_path,CSfactors_file),index=False)
		# print(CSfactors_file+'written')
