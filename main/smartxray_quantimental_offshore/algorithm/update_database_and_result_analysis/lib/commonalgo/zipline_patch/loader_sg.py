...
from .benchmarks_sgm import get_aqm_bm_returns
# from . import treasuries, treasuries_can
from zipline.data import treasuries, treasuries_can
...
def get_yield_markort(iuid =[],start_date = None, end_date=None):

    data = get_data(iuids=[iuid], tags=[4], period='D', start_date=start_date, end_date=end_date)[(iuid,'4')]
    data.index = pd.DatetimeIndex(data.index).tz_localize(tz='UTC')
    data = data.astype('float')/100.
    return pd.DataFrame(data.rename('10year')).rename_axis('Time Period')
...
def ensure_benchmark_data(symbol, first_date, last_date, now, trading_day,
    ...
    try:
        ...
        data = get_aqm_bm_returns(symbol)
...
def ensure_treasury_data(symbol, first_date, last_date, now, environ=None):
    ...
    try:
        ...
        data = get_yield_markort('US_30_TREA10YIELD',
