...
from ..zipline_patch.csvdata_hk import csvaqm_equities
...
class CNFuturesBundle(BaseBundle):
	def register_bundle(self, fromcsv=False, summaryfolder=None, freq_list=['daily']):
		symbols_path = os.path.join(summaryfolder, 'Summary.csv')
		summary = pd.read_csv(symbols_path)
		symbols = tuple(summary['Name'].tolist())
		all_minutes = 390
	...
	def update_bundle(self, fromcsv=False, summaryfolder=None, freq_list=['daily']):
		self.register_bundle(fromcsv, summaryfolder, freq_list=freq_list)
		ingest(self.name)
		print ('Data bundle <{}> update complete'.format(self.name))