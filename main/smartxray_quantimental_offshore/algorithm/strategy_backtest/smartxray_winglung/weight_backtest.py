...
from algorithm.strategy_backtest import s1_bundle_injection as s1
from algorithm.strategy_backtest import s2_funds_pool as s2
from algorithm.strategy_backtest import s3_weighting as s3
from algorithm.strategy_backtest import s4_back_testing as s4
...
def distribution(sector_preference, region_preference, name, weight_info, timestamp, sector_allocation, country_allocation, credit_quality_allocation):
    ...
    # if sector_preference != 'A1' or region_preference !='B1':
    #     distribution_dict = {'distributions':[pd.DataFrame([1]), pd.DataFrame([1]), pd.DataFrame([1])],
    #                          'fund_type':{timestamp:[{'FundSectorName': 'Basic Materials', 'FundSectorRate': 0}]}}
...
def weighting_backtesting(start_date, end_date, output_path, test_dict, type):
    ...
    if type == 'skip_bundle':
        pass
    elif type == 'skip_download_bundle':
        s1.bundle_injection()
    else:
            ...
            for sector_preference in test_dict['sector_preference']:
                ...
                if equity_pct < 0.15:
                    backtest_result_tmp = s4.index_backtesting(start=start_date, end=end_date,
                                                                equity_pct = equity_pct,
                                                                sector_preference = 'A1', region_preference= 'B1',
                                                                commission_fee=0, initial_capital=1000000,
                                                                rebalance_freq='6M', model_freq='12M')
                else:
                    backtest_result_tmp = s4.index_backtesting(start=start_date, end=end_date,
                                                                equity_pct=equity_pct,
                                                                sector_preference = sector_preference, region_preference= region_preference,
                                                                commission_fee=0, initial_capital=1000000,
                                                                rebalance_freq='6M', model_freq='12M')
                
                if equity_pct < 0.15:
                    info_1 = str(int(equity_pct * 100)) + '-A1-B1'
                else:
                    info_1 = str(int(equity_pct * 100)) + '-' + sector_preference + '-' + region_preference
                bt_result_path = join(result_path, 'backtest', "6M-12M", "PV" + "_" + info_1 + ".csv")
                backtest_result_tmp = pd.read_csv(bt_result_path, index_col=0, parse_dates=[0])
                ...
                data_dict_tmp = distribution(sector_preference=sector_preference, region_preference=region_preference,
                                                name = info, weight_info=recent_weight_tmp, timestamp = recent_timestamp_tmp,
    ...
    for i in range(len(weight_df_list)):
        ...
        weight_df.to_excel(writer, sheet_name=name_list[i])
    ...
    eng_name_df['ms_secid'] = eng_name_df['ms_secid'].map(lambda x:secid_mapping[x] if x in secid_mapping.index else x)
    ...
    ######################## Change all iuid to original secid #################################################
    algo_secid_mapping = pd.Series(secid_mapping.index, index=secid_mapping.values)

    post2database_to_algo_team = pd.read_csv(r'D:\script\smartxray_quantimental_wlb\result\todatabase Final Version\post2database.csv')
    algo_weight_hist = post2database_to_algo_team['weight']
    
    correct_algo_weight_hist = []
    for algo_weight in algo_weight_hist:
        algo_weight_dict = yaml.load(algo_weight.replace("'", "\""), Loader=yaml.FullLoader)
        algo_weight_df = pd.DataFrame.from_dict(algo_weight_dict).T
        algo_weight_df.columns = algo_weight_df.columns.str[6:]
        
        algo_weight_df.columns = algo_weight_df.columns.map(lambda x:algo_secid_mapping.loc[x] if x in algo_secid_mapping.index else x)
        
        algo_weight_df.columns = pd.Series(algo_weight_df.columns).replace('F0GBR053YB', 'F0GBR04SSN')
        algo_weight_df.columns = pd.Series(algo_weight_df.columns).replace('F0GBR053E5', 'F0GBR04AMI')
        algo_weight_df.columns = pd.Series(algo_weight_df.columns).replace('F0GBR053DP', 'F0GBR04K8L')
        algo_weight_df.columns = pd.Series(algo_weight_df.columns).replace('F00000VTAH', 'F00000T2AE')
        algo_weight_df.columns = pd.Series(algo_weight_df.columns).replace('F0GBR04MRX', 'F0GBR04MRU')
        
        # algo_weight_df.columns = 'HK_20_' + algo_weight_df.columns
        correct_algo_weight_dict = {k: v.dropna().to_dict() for k,v in algo_weight_df.T.items()}
        correct_algo_weight_hist.append(str(correct_algo_weight_dict))
        
    post2database_to_algo_team['weight'] = correct_algo_weight_hist
    post2database_to_algo_team.to_csv(join(result_path, 'todatabase Final Version', 'post2database_to_algo.csv'), index=False)
...
if __name__ == '__main__':
    ...
    weighting_backtesting(start_date = '2016-02-29', end_date = '2021-01-08', output_path=join(result_path, 'todatabase'),
                          test_dict=test_dict, type = '')
    end = datetime.now()
