...
# def weight_optimize(weights_df_tmp, price_df, date, bond_list_tmp, sector_clas, lower_bound, equity_pct):
#     weights_df_tmp_out = weights_df_tmp.copy()
#
#     fund_prices = price_df.loc[:, weights_df_tmp['Funds']]
#     fund_prices = fund_prices[np.logical_and(fund_prices.index >= date - relativedelta(months=12), fund_prices.index < date)]
#     fund_weekly_prices = fund_prices.asfreq('W-MON', method='pad')
#     fund_weekly_returns = fund_weekly_prices.pct_change().fillna(0)
#
#     control_class = [c for c in weights_df_tmp.index if c not in bond_list_tmp + [sector_clas]]
#     fix_class = [c for c in weights_df_tmp.index if c not in control_class]
#     weights_df_tmp.loc[control_class, 'LB'] = weights_df_tmp.loc[control_class, 'Weight'] - 0.02
#     weights_df_tmp.loc[fix_class, 'LB'] = lower_bound
#     weights_df_tmp.loc[control_class, 'UB'] = weights_df_tmp.loc[control_class, 'Weight'] + 0.02
#     weights_df_tmp.loc[fix_class, 'UB'] = 1
#     weights_df_tmp['LB'] = np.where(weights_df_tmp['LB'] < lower_bound, lower_bound, weights_df_tmp['LB'])
#     weights_df_tmp.loc[bond_list_tmp, 'BD'] = 1
#     weights_df_tmp['BD'] = weights_df_tmp['BD'].fillna(0)
#
#     tmp = fund_weekly_returns.copy()
#     tickers = tmp.columns.values
#     cov = pd.DataFrame(None, index=tickers, columns=tickers)
#     for x in tickers:
#         for y in tickers:
#             cov.loc[x, y] = pd.np.dot(tmp[x], tmp[y])
#
#     expected_return = tmp.mean().values.astype(float) * 252
#     lmb = 2.5
#     n = len(expected_return)
#     P = np.dot(lmb, cov.values.astype(float))
#     q = expected_return * (-1)
#
#     lb = []
#     ub = []
#     g = []
#     for c in control_class:
#         weights_df_tmp[c] = 0
#         weights_df_tmp.loc[c, c] = 1
#         g.append(weights_df_tmp[c].values.astype(float).reshape(1, n))
#         lb.append(np.ones(1).reshape(1,1) * weights_df_tmp.loc[c, 'LB'])
#         ub.append(np.ones(1).reshape(1,1) * weights_df_tmp.loc[c, 'UB'])
#     G = np.vstack([-np.vstack(g), np.vstack(g)]).astype(float)
#     h = np.vstack(lb + ub).astype(float)
#
#     # A = np.vstack([np.ones(n).reshape(1, n), weights_df_tmp['BD'].values.astype(float).reshape(1, n)])
#     # b = np.vstack([np.ones(1).reshape(1,1), np.ones(1).reshape(1,1) * (1 - equity_pct)])
#     A = np.vstack([np.ones(n).reshape(1, n)])
#     b = np.vstack([np.ones(1).reshape(1,1)])
#     for c in fix_class:
#         weights_df_tmp[c] = 0
#         weights_df_tmp.loc[c, c] = 1
#         A = np.vstack([A, weights_df_tmp[c].values.astype(float).reshape(1, n)])
#         b = np.vstack([b, weights_df_tmp.loc[c, 'Weight'] * np.ones(1).reshape(1,1)])
...
#     	cvxopt.matrix(b)
#     )
#     weights_df_tmp_out['raw_Weight'] = weights_df_tmp['Weight']
#     weights_df_tmp_out['Weight'] = optimized['x']

def weight_optimize(weights_df_tmp, price_df, date, bond_list_tmp, sector_clas, lower_bound, equity_pct):
    weights_df_tmp_out = weights_df_tmp.copy()
    fund_prices = price_df.loc[:, weights_df_tmp['Funds']]
    fund_prices = fund_prices[np.logical_and(fund_prices.index >= date - relativedelta(months=12), fund_prices.index < date)] # Lookback one year of fund prices
    fund_weekly_prices = fund_prices.asfreq('W-MON', method='pad')
    fund_weekly_returns = fund_weekly_prices.pct_change().fillna(0)

    control_class = [c for c in weights_df_tmp.index if c not in bond_list_tmp + [sector_clas]] # All equity funds need to be controlled
    fix_class = [c for c in weights_df_tmp.index if c not in control_class] # All bond funds are fixed in weight
    weights_df_tmp.loc[control_class, 'LB'] = weights_df_tmp.loc[control_class, 'Weight'] - 0.02
    weights_df_tmp.loc[fix_class, 'LB'] = lower_bound
    weights_df_tmp.loc[control_class, 'UB'] = weights_df_tmp.loc[control_class, 'Weight'] + 0.02
    weights_df_tmp.loc[fix_class, 'UB'] = 1
    weights_df_tmp['LB'] = np.where(weights_df_tmp['LB'] < lower_bound, lower_bound, weights_df_tmp['LB'])
    weights_df_tmp['UB'] = np.where(weights_df_tmp['UB'] < weights_df_tmp['LB'], weights_df_tmp['LB'], weights_df_tmp['UB'])
    ...
    for x in tickers:
        for y in tickers:
            cov.loc[x, y] = pd.np.dot(tmp[x], tmp[y])
    cov_control = pd.DataFrame(None, index=control_class, columns=control_class)
    for x in cov_control:
        for y in cov_control:
            cov_control.loc[x, y] = pd.np.dot(tmp[weights_df_tmp.loc[x, 'Funds']], tmp[weights_df_tmp.loc[y, 'Funds']])
    mc_weight = weights_df_tmp.loc[control_class, 'Weight'].values.astype(float)
    lmb = 2.5
    expected_return_control = lmb * np.dot(cov_control, mc_weight)
    expected_return = fund_prices.pct_change().mean() * 252
    expected_return[weights_df_tmp.loc[control_class, 'Funds']] = expected_return_control
    # expected_return = tmp.mean().values.astype(float) * 252
    # sr_fund_prices = fund_prices[np.logical_and(fund_prices.index >= date - relativedelta(months=4), fund_prices.index < date)]
    # sr = sr_fund_prices.iloc[-1, :] / sr_fund_prices.iloc[0, :] - 1
    # mmt_fund_prices = fund_prices[np.logical_and(fund_prices.index >= date - relativedelta(months=12), fund_prices.index < date - relativedelta(months=4))]
    # mmt = mmt_fund_prices.pct_change().mean() * 252
    fund_cum_max = fund_prices.cummax()
    fund_drawdown = 1 - fund_prices / fund_cum_max
    mdd = fund_drawdown.max()
    # expected_return = -0.1 * sr + 0.05 * mmt - 0.1 * mdd
    # expected_return = expected_return - 0.01 * mdd
    # lmb = 12
    n = len(expected_return)
    P = np.dot(lmb, cov.values.astype(float))
    q = expected_return.values.reshape(1, n) * (-1)
    ...
    for c in control_class:
        ...
        weights_df_tmp.loc[c, c] = 1
        g.append(weights_df_tmp[c].values.astype(float).reshape(1, n))
        lb.append(weights_df_tmp.loc[c, 'LB'])
        ub.append(weights_df_tmp.loc[c, 'UB'])
    G = np.vstack([-np.vstack(g), np.vstack(g)]).astype(float)
    h = np.vstack([-np.vstack(lb), np.vstack(ub)]).astype(float).reshape(len(G),) 
    ...
    b = b.reshape(len(A), )
    x = cp.Variable(n)
    prob = cp.Problem(cp.Minimize((1 / 2) * cp.quad_form(x, P) + q @ x), # cp.quad_form(x, P) = x^T P x
                      [G @ x <= h, # Make sure the wegihts are less than upper bound & negative of weights are less than negative lower bound (that is all weight larger than lower bound)
                       A @ x == b] # Make sure all the weights sums to one and the bond wieghts equal to the fixed rate
                      )
    prob.solve()
    weights_df_tmp_out['raw_Weight'] = weights_df_tmp['Weight']
    weights_df_tmp_out['Weight'] = x.value
    return weights_df_tmp_out[['Funds', 'Weight']]
...
def compute_weights_of_bond_funds(bond_fund_price,
    ...
    if method=='risk_parity':
        ...
        fund_daily_prices = bond_fund_price.copy()
        fund_daily_prices = fund_daily_prices.fillna(method='ffill')
        fund_cum_max = fund_daily_prices.cummax()
        fund_drawdown = 1 - fund_daily_prices / fund_cum_max
        mdd = fund_drawdown.max()
        mdd[mdd < 0.01] = 0.01
        inverse_mdd = mdd.apply(lambda x: 1. / x)

        # weights_of_bond_funds = inverse_std / inverse_std.sum()
        weights_of_bond_funds = inverse_mdd / inverse_mdd.sum()
...
# To ensure that the weight of bonds would not exceed the upper and lower bounds
...
def compute_weights(start_date, end_date, equity_pct, sector_preference, region_preference, lower_bound, price_df):
    ...
    fundsfile_path = join(pool_path, '12M')
    ...
    if equity_pct < 0.25:
        bond_list = ['Global IG', 'Global IG 2', 'US IG', 'Other IG']
    elif equity_pct >=0.25 and equity_pct < 0.65:
        bond_list = ['Global IG', 'Global IG 2', 'US IG', 'Other IG', 'High Yield']
    elif equity_pct >= 0.65 and equity_pct < 0.8:
        bond_list = ['Global IG', 'Global IG 2', 'US IG','High Yield']
    ...
    for date in dates_list:
        ...
        if equity_pct < 0.15:
            ...
            weights_of_region_df = weights_of_region_df_tmp.copy()
        ...
        if equity_pct >= 0.27:
            weights_df_tmp = weight_optimize(weights_df_tmp, price_df, date, bond_list_tmp, sector_clas, lower_bound, equity_pct)
...
if __name__ == '__main__':
    ...
    for equity_pct in [0.07, 0.17, 0.27, 0.45, 0.65]:
        compute_weights(start_date = '2016-02-29', end_date = '2020-12-31', equity_pct = equity_pct, sector_preference = 'A1',
