...
def create_backtest_engine_files(stocks, strategy_name, strategy_config_path, result_path, start_dt, end_dt, cash,
    ...
    params['data'] = {
        "STOCKS": {
            "DataCenter": "LocalCSV",
            "DataCenterArgs":{
                "main_dir": os.path.join(addpath.data_path, 'benchmark', "bundles"),
                "fxrate_dir": os.path.join(addpath.data_path, 'benchmark',  "fxrate"),
                "int_dir": os.path.join(addpath.data_path, 'benchmark', "interest"),
                "info": os.path.join(addpath.data_path, 'benchmark', "info", "info.csv")
            },
            "Fields": "OHLC",
            "Frequency": "DAILY"
        }
