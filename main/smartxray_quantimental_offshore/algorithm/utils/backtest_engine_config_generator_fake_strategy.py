...
def create_backtest_engine_files(stocks, strategy_name, strategy_config_path, result_path, start_dt, end_dt, cash,
                                 commission_rate, window_size, save_dm_to_local, use_local_dm, real_strategy_name):
    ...
    params['data'] = {
        "STOCKS": {
            "DataCenter": "LocalCSV",
            "DataCenterArgs":{
                "main_dir": os.path.join(addpath.data_path, 'bundles', 'daily'),
                "fxrate_dir": os.path.join(addpath.data_path, 'quantcycle_backtest', "fxrate"),
                "int_dir": os.path.join(addpath.data_path, 'quantcycle_backtest', "interest"),
                "info": os.path.join(addpath.data_path, 'quantcycle_backtest', "info", "info.csv")
            },
            "Fields": "OHLC",
            "Frequency": "DAILY"
        }
    ...
    params['dm_pickle'] = {
      ...
      "save_name": real_strategy_name + "_dm_pickle",
