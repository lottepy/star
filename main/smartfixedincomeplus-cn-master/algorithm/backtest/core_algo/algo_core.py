...
class score_wt_combine(BaseStrategy):
...
class mth_wt_ashare(BaseStrategy):
    ...
    def on_data(self, data_dict: dict, time_dict: dict, ref_data_dict: dict, ref_time_dict: dict):
        ...
        if self.rebalance_count < len(self.formation_dates):
            if current_date >= self.formation_dates[self.rebalance_count]:
                ...
                score = pd.read_csv(os.path.join(self.weight_path, self.formation_dates[self.rebalance_count].strftime("%Y-%m-%d") + ".csv"), index_col=0)
...
class fix_weight_combine(BaseStrategy):

    def init(self):
        self.rebalance_count = 0
        self.weight_path = os.path.join(addpath.data_path, "Ashare", "weights", "ew_ashare", "qtr", "1")
        formation_dates = os.listdir(self.weight_path)
        self.formation_dates = [datetime.strptime(formation_date[:-4], "%Y-%m-%d") for formation_date in formation_dates]
        self.holding_shares = np.zeros(len(self.symbol_batch)).reshape(1, -1)
    ...
    def on_data(self, data_dict: dict, time_dict: dict, ref_data_dict: dict, ref_time_dict: dict):
        ...
        if self.rebalance_count < len(self.formation_dates):
            if current_date >= self.formation_dates[self.rebalance_count]:
                ...
                weight = {}
                # weight['pv_ashare_rf_qtr'] = 0.2
                weight['pv_ashare_rf_sw2_qtr_timed'] = 0.2
                weight['pv_bond'] = 0.8

                holding_shares_list_tmp = pd.Series()
                for stock in weight.keys():
                    holding_shares_list_tmp[stock] = int(self.portfolio_manager.pv * weight[stock] / close_df.loc[current_date, stock])
