...
def cal_factors(symbol):
    ...
    trading_data = trading_data.resample('1M').last().ffill()
    trading_data['fwd_return_qtr'] = trading_data['PX_LAST'].shift(-3) / trading_data['PX_LAST'] - 1
    trading_data['fwd_return_semiyear'] = trading_data['PX_LAST'].shift(-6) / trading_data['PX_LAST'] - 1
    trading_data['fwd_return_annyear'] = trading_data['PX_LAST'].shift(-12) / trading_data['PX_LAST'] - 1
    ...
    financial_data = financial_data.dropna(subset=['report_date'])

    financial_report_date_path = os.path.join(addpath.data_path, "Ashare", "date", symbol + ".csv")
    financial_report_date = pd.read_csv(financial_report_date_path, index_col='Date', parse_dates=['Date', 'STMTACTDATE'])

    financial_data = financial_data.reset_index()
    financial_data = financial_data.set_index('report_date')
    financial_data.index.name = 'report_date'

    financial_report_date_all = financial_data[['end_date']]
    financial_data['end_date'] = financial_report_date['STMTACTDATE']
    financial_data['end_date'] = financial_data['end_date'].fillna(financial_report_date_all['end_date'])

    financial_data.sort_index(inplace=True)

    financial_data = financial_data.dropna(subset=['TINCOMESTATEMENTQ_9'])

    financial_data['MARKET_CAP'] = trading_data['MARKET_CAP']
    ...
    # Resample monthly
    financial_data = financial_data.reset_index()
    financial_data = financial_data.set_index('end_date')
    financial_data.index.name = 'end_date'
    if financial_data.index[-1] != trading_data.index[-1]:
        financial_data = pd.concat([financial_data, financial_data.iloc[[-1], :]])
        financial_data['idx'] = financial_data.index
        financial_data.iloc[-1, -1] = trading_data.index[-1]
        financial_data = financial_data.set_index('idx')
        financial_data.index.name = 'end_date'
    financial_data = financial_data.resample('1M').last().ffill()

    # Size
    financial_data['MARKET_CAP'] = trading_data['MARKET_CAP']
    financial_data['EQY_SH_OUT'] = trading_data['EQY_SH_OUT']
    financial_data['fwd_return_qtr'] = trading_data['fwd_return_qtr']
    financial_data['fwd_return_semiyear'] = trading_data['fwd_return_semiyear']
    financial_data['fwd_return_annyear'] = trading_data['fwd_return_annyear']
    ...
    financial_data['TPTP'] = financial_data['RATINGTARGETPRICE'] / trading_data['PX_LAST_RAW']

    # Valuation
    financial_data['EPTTM'] = financial_data['TINCOMESTATEMENTQ_60_TTM'] / trading_data['MARKET_CAP']
    financial_data['SPTTM'] = financial_data['TINCOMESTATEMENTQ_9_TTM'] / trading_data['MARKET_CAP']
    ...
    # Empire_Building
    financial_data['ASSET_gr_1yr'] = financial_data['TBALANCESTATEMENT_74'] / financial_data['TBALANCESTATEMENT_74'].shift(12) - 1
    financial_data['BV_gr_1yr'] = financial_data['TBALANCESTATEMENT_141'] / financial_data['TBALANCESTATEMENT_141'].shift(12) - 1
    financial_data['SH_gr_1yr'] = financial_data['EQY_SH_OUT'] / financial_data['EQY_SH_OUT'].shift(12) - 1
    financial_data['ASSET_gr_2yr'] = financial_data['TBALANCESTATEMENT_74'] / financial_data['TBALANCESTATEMENT_74'].shift(24) - 1
    financial_data['BV_gr_2yr'] = financial_data['TBALANCESTATEMENT_141'] / financial_data['TBALANCESTATEMENT_141'].shift(24) - 1
    financial_data['SH_gr_2yr'] = financial_data['EQY_SH_OUT'] / financial_data['EQY_SH_OUT'].shift(24) - 1
    financial_data['ASSET_Abnormal'] = financial_data['TBALANCESTATEMENT_74'] / ((financial_data['TBALANCESTATEMENT_74'].shift(12) + financial_data['TBALANCESTATEMENT_74'].shift(24)) / 2)  - 1
    financial_data['BV_Abnormal'] = financial_data['TBALANCESTATEMENT_141'] / ((financial_data['TBALANCESTATEMENT_141'].shift(12) + financial_data['TBALANCESTATEMENT_141'].shift(24)) / 2)  - 1
    financial_data['SH_Abnormal'] = financial_data['EQY_SH_OUT'] / ((financial_data['EQY_SH_OUT'].shift(12) + financial_data['EQY_SH_OUT'].shift(24)) / 2)  - 1
    ...
    # result = financial_data[['ARTOS', 'sales', 'cashflow_generation', 'profitability', 'profitability_growth', 'sales_growth',
    ...
    financial_data.to_csv(result_path)
