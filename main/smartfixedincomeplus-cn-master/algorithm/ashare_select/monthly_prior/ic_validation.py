...
def IC_helper(industry_level, formation_date, horizon):
    ...
    selected_stock_path = os.path.join(addpath.data_path, "Ashare", "selected_stocks", industry_level, horizon, formation_date.strftime('%Y-%m-%d') + ".csv")
    ...
    ic_path = os.path.join(addpath.data_path, "Ashare", "monthly_prior", "IC", industry_level, horizon)
...
def IC_formation(industry_level, horizon):
    ss_path = os.path.join(addpath.data_path, "Ashare", "selected_stocks", industry_level, horizon)

    formation_dates = os.listdir(ss_path)
    ...
    for formation_date in formation_dates:
        ...
        pool.apply_async(IC_helper, args=(industry_level, formation_date, horizon, ))
...
def trading_factor_selection_helper(industry_level, rebalance_date, horizon):
    ...
    if os.path.exists(selected_trading_factors_path):
        pass
    else:
        os.makedirs(selected_trading_factors_path)
...
def trading_factor_selection(start, end, horizon, industry_level):
    rebalance_dates = pd.date_range(start, end, freq='1M')
    pool = multiprocessing.Pool()
    for rebalance_date in rebalance_dates:
        # trading_factor_selection_helper(rebalance_date)
        pool.apply_async(trading_factor_selection_helper, args=(industry_level, rebalance_date, horizon, ))
        print('Sub-processes begins!')
    pool.close()
    pool.join()
    print('Sub-processes done!')
...
if __name__ == "__main__":
    ...
    # horizon = "semiyear"
    industry_level = "SW2"
    IC_formation(industry_level, horizon)
    start = '2013-04-30'
    end = '2020-11-30'
    trading_factor_selection(start, end, horizon, industry_level)
