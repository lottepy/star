...
def generate_strategy_pool(strategy_name):
    ...
    pool_setting = {
        ...
        "strategy_module":
            "strategy.uat_stocks.algorithm.external_signal_strategy",
...
def generate_portfolio_strategy_pool(strategy_name_list):
    ...
    pool_setting = {
        "symbol": symbols,
        "strategy_module":
            "strategy.uat_stocks.algorithm.portfolio_strategy",
...
def strategy_subscription(config, strategy_pool_df, last_level):
    req_url = "https://172.29.39.140:1234/strategy_subscription"
...
def receive_result(strategy):
    req_url = "https://172.29.39.140:1234/result"
...
def run(strategy_name):
    config = json.load(open('strategy/uat_stocks/configs/external_signal_template.json'))
    new_config = {
        ...
        "end_day": END_DATE[2],
        "result_output": {
            "save_dir": f"strategy/uat_stocks/results/{strategy_name}",
            "status_dir": f"strategy/uat_stocks/results/{strategy_name}"
        },
        "engine": {
...
def run_portfolio(strategy_name_list):
    config = json.load(open('strategy/uat_stocks/configs/portfolio.json'))
    secondary_data = {}
    for strategy_name in strategy_name_list:
        one_secondary_data = {
            "DataCenter": "ResultReader",
            "DataCenterArgs": {
                "DataPath": f"strategy/uat_stocks/results/{strategy_name}/result"
            },
            "Fields": ["position"],
            "Frequency": "DAILY"
        }
    ...
    new_config = {
        ...
        "end_day": END_DATE[2],
        "result_output": {
            "save_dir": "strategy/uat_stocks/results/portfolio",
            "status_dir": "strategy/uat_stocks/results/portfolio"
        },
        "engine": {
