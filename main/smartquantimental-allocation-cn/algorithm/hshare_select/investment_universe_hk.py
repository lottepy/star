...
def investment_universe_HK(symbol_list, trading_data_total, formation_date, market_cap_threshold):
	...
	investment_univ.to_csv(os.path.join(hk_path, "investment_universe", formation_date.strftime('%Y-%m-%d') + ".csv"),
						   index=False)
...
def investment_univ_helper(stock_list, formation_date, hk_path, trading_data_total):
	...
	investment_universe_HK(symbol_list, trading_data, formation_date,market_cap_threshold)
...
if __name__ == "__main__":
	...
	formation_dates = pd.date_range('2012-12-31', '2020-11-30', freq='1M')
	...
	if os.path.exists(investment_univ_path):
		pass
	else:
		os.makedirs(investment_univ_path)
