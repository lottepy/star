...
def optimal_weighting(weights_before_cap_df, funds_price_df_tmp, index_volume_df, date, lower_bound):
    ...
    for cate in weights_before_cap_df.index.tolist():
        if cate not in ['投资级债券', '高收益债', '政策性银行债券', '货币']:
...
def compute_weights(start_date, end_date, equity_pct, sector_preference, invest_length, lower_bound, price_df):
    bl_weights_path = join(addpath.data_path, 'bl_weights', 'GZNSH')
    pool_path = join(addpath.data_path, 'pool', 'GZNSH')
    ...
    rebalance_dates_df = pd.read_csv(join(addpath.config_path, 'GZNSH', 'rebalance_dates.csv'), parse_dates=[0])
    ...
    invest_length_df = pd.read_csv(join(addpath.config_path, 'GZNSH', 'invest_length.csv'), index_col=0)
    ...
    money_list = ['货币']
    bond_list = ['投资级债券', '高收益债', '政策性银行债券']
    # alternative_list = ['另类资产']

    sector_preference_dict = {
        'A2': '医疗',
        'A3': '消费',
        'A4': '金融',
        'A5': '科技'
    ...
    for date in dates_list:
        ...
        if equity_pct<=0.2:
            sector_pct = 0.243 * equity_pct_adj
        else:
            sector_pct = 0.3 * equity_pct_adj


        if equity_pct == 0.2:
            if '消费' in funds_df_tmp['class'].tolist():
                weights_of_sector_df = funds_df_tmp.set_index('class').copy().loc[['消费'], ['Funds']]
                weights_of_sector_df['Weight'] = sector_pct
            else:
            ...
            if sector_preference != 'A1':
                clas = sector_preference_dict[sector_preference]
                delta_sector_pct = (0.025 - 0.018) / (0.7 - 0.4) * (equity_pct_adj - 0.4) + 0.018
                if clas in weights_of_sector_df_tmp.index:
                    weights_of_sector_df_tmp.loc[clas, 'Weight'] = weights_of_sector_df_tmp.loc[
                                                                       clas, 'Weight'] + delta_sector_pct
                    other_clas = [c for c in weights_of_sector_df_tmp.index if c != clas]
                    weights_of_sector_df_tmp.loc[other_clas, 'Weight'] \
                        = weights_of_sector_df_tmp.loc[other_clas, 'Weight'] - delta_sector_pct / len(
                        weights_of_sector_df_tmp.loc[other_clas, 'Weight'])
                else:
                    pass
            # # number_of_funds = weights_of_sector_df_tmp.shape[0]
        ...
        money_weight =  (0.0831 - 0.0412) / (0.1 - 0.8) * (equity_pct_adj - 0.8) + 0.0412
        ...
        special_bond = '高收益债'
        other_clas = [c for c in weights_of_bond_funds_df_tmp.index if c != special_bond]
        other_bond_min_weight = weights_of_bond_funds_df_tmp.loc[other_clas, 'Weight'].min()
        highret_weight_tmp = weights_of_bond_funds_df_tmp.loc[special_bond, 'Weight']
        if other_bond_min_weight < highret_weight_tmp:
            weights_of_bond_funds_df_tmp.loc[special_bond, 'Weight'] = other_bond_min_weight
            weights_of_bond_funds_df_tmp.loc[other_clas, 'Weight'] = \
                weights_of_bond_funds_df_tmp.loc[other_clas, 'Weight'] + (
                            highret_weight_tmp - other_bond_min_weight) / len(other_clas)

        if special_bond in weights_of_bond_funds_df_tmp.index:
            n = len(weights_of_bond_funds_df_tmp)
            upper_bound_for_highret = ((0.0549 - 0.1151) / (0.7 - 0.2) * (equity_pct_adj - 0.2) + 0.1151) * 3 / n
            highret_weight_tmp = weights_of_bond_funds_df_tmp.loc[special_bond, 'Weight']

            if highret_weight_tmp > upper_bound_for_highret:
                weights_of_bond_funds_df_tmp.loc[special_bond, 'Weight'] = upper_bound_for_highret

                weights_of_bond_funds_df_tmp.loc[other_clas, 'Weight'] \
                    = weights_of_bond_funds_df_tmp.loc[other_clas, 'Weight'] \
                      + (highret_weight_tmp - upper_bound_for_highret) / (n - 1)
            else:
                pass
        else:
            pass
...
if __name__ == '__main__':
    ...
    for equity_pct in [0.2, 0.4, 0.5, 0.6, 0.7]:
        for sector in ['A1', 'A2', 'A3', 'A4', 'A5']:
            for invest_length in ['B1', 'B2', 'B3', 'B4', 'B5']:
                compute_weights(start_date='2014-04-30', end_date='2020-12-31', equity_pct=equity_pct, sector_preference=sector,
                                invest_length=invest_length, lower_bound=0.0263, price_df=price_df)