...
def investment_universe(symbol_list, trading_data_total, formation_date,input_path,underlying_type,
										  market_cap_threshold,min_price):
...
def investment_univ_helper(stock_list, formation_date, input_path,underlying_type,trading_data_total):
    ...
    if underlying_type == "US_STOCK":
        min_price=5
        market_cap_threshold = 0.975
    elif underlying_type == "HK_STOCK":
        min_price = 1   #待定
        market_cap_threshold = 0.9    #待定
    elif underlying_type == "CN_STOCK":
        min_price = 5
        market_cap_threshold = 0.99
    investment_univ = investment_universe(symbol_list, trading_data, formation_date,input_path,underlying_type,
										  market_cap_threshold,min_price)
    print(investment_univ)
    investment_univ = pd.DataFrame(investment_univ, columns=['symbol'])
    investment_univ.to_csv(os.path.join(input_path, "investment_univ", formation_date.strftime('%Y-%m-%d') + ".csv"),
                           index=False)
    print(formation_date.strftime('%Y-%m-%d') + ".csv"+"written")

if __name__ == "__main__":
    underlying_types=['CN_STOCK','US_STOCK']
    underlying_types=['US_STOCK']
    for underlying_type in underlying_types:
        ...
        stock_list =[]
        # stock_list=pd.read_csv(os.path.join(input_path, "symbol_list.csv"))['symbol'].tolist()
        trading_path=os.path.join(input_path, "trading")
        os.chdir(trading_path)
        csv_name_list = os.listdir()
        if '.DS_Store' in csv_name_list:
            csv_name_list.remove('.DS_Store')
        trading_data_total = {}
        for ticker in csv_name_list:
            ticker=ticker[:-4]
            print(ticker)
            data_in=pd.read_csv(os.path.join(input_path, "trading", ticker+'.csv'), parse_dates=[0], index_col=0)
            stock_list.append(ticker)
            trading_data_total[ticker] = pd.read_csv(os.path.join(input_path, "trading", ticker+'.csv'), parse_dates=[0], index_col=0)

        with open(os.path.join(input_path, "trading_date_dict.pickle"), "wb") as fp:  # Pickling
            pickle.dump(trading_data_total, fp, protocol=pickle.HIGHEST_PROTOCOL)
        with open(os.path.join(input_path, "trading_date_dict.pickle"), "rb") as fp:  # Pickling
            trading_data_total = pickle.load(fp)
            stock_list = list(trading_data_total.keys())

        trading_data_total_keys = list(trading_data_total.keys())
        len_stock=len(trading_data_total_keys)
        len_dict=round(len_stock / 4)
        ...
        end_date='2020-12-31'
        formation_dates = pd.date_range(start_date,end_date, freq='1M')
        pool = multiprocessing.Pool()
        for formation_date in formation_dates:
            print(formation_date)
            # pool.apply_async(investment_univ_helper, args=(stock_list, formation_date, input_path, trading_data_total_1,
            # 											   trading_data_total_2, trading_data_total_3, trading_data_total_4))
            # print('Sub-processes begins!')
            investment_univ_helper(stock_list, formation_date, input_path,underlying_type, trading_data_total)
        pool.close()
        pool.join()
