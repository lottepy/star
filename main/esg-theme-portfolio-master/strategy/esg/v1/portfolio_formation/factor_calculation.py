...
def cal_factors(symbol, trading_data, financial_data, reference_data, rebalance_freq):
	...
	if temp.shape[0] > 0:
		...
		temp['ILLIQ'] = temp['ret_daily'].map(lambda x: abs(x)) / (temp['PX_VOLUME'] * temp['PX_LAST_RAW'])
	...
	if temp.shape[0] > 0:
		...
		# Profitability
		temp['ROE'] = temp['NET_INCOME'] / temp['TOT_EQUITY']
		temp['ROA'] = temp['NET_INCOME'] / temp['BS_TOT_ASSET']
		temp['CFOA'] = temp['CF_CASH_FROM_OPER'] / temp['BS_TOT_ASSET']

		# Valuation
		if result.shape[0] > 0:
			temp['BTM'] = temp['TOT_EQUITY'] / (temp['EQY_SH_OUT'] * temp['PX_LAST_RAW'])
			temp['ETP'] = temp['NET_INCOME'] / (temp['EQY_SH_OUT'] * temp['PX_LAST_RAW'])
			temp['STP'] = temp['SALES_REV_TURN'] / (temp['EQY_SH_OUT'] * temp['PX_LAST_RAW'])
			temp['CFTP'] = temp['CF_CASH_FROM_OPER'] / (temp['EQY_SH_OUT'] * temp['PX_LAST_RAW'])
		else:
			temp['BTM'] = np.nan
			temp['ETP'] = np.nan
			temp['STP'] = np.nan
