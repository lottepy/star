...
def theme_conditions(last_period_factors,symbol_industry, investment_univ,factor_list):
    criteria = {
        factor_list[0]: ['Positive',0.2],
        factor_list[1]: ['Positive',0.2],
        factor_list[2]: ['Positive', 0.2],
        factor_list[3]: ['Positive', 0.2],
        factor_list[4]: ['Positive', 0.2],
        factor_list[5]: ['Positive', 0.2],
    ...
    selection_list = []
    # Measure Z score within each industry
    for ind in ind_list:
        ...
        temp = temp[temp['ind_zscore'].rank(ascending=True) <= 2] # Only select the first 5 stocks within the industry
    ...
    print('Number of stocks in final ranking : {}'.format(selection_tmp.shape[0]))
