...
def create_backtest_engine_files(stocks, strategy_name, strategy_config_path, result_path, start_dt, end_dt, cash,
    ...
    params['data'] = {
        "STOCKS": {
            "DataCenter": "LocalCSV",
            "DataCenterArgs":{
                "main_dir": os.path.join(addpath.data_path, "bundles"),
                "fxrate_dir": os.path.join(addpath.data_path,  "fxrate"),
                "int_dir": os.path.join(addpath.data_path, "interest"),
                "info": os.path.join(addpath.data_path, "info", "info.csv")
            },
            "Fields": "OHLC",
            "Frequency": "DAILY"
        }
    ...
    params['ref_data'] = {
        "mktcap": {
            "DataCenter": "LocalCSV",
            "DataCenterArgs": {
                "dir": os.path.join(addpath.data_path, "reference_data", "mktcap")
            },
            "Symbol":  stocks,
            "StartDate": {
