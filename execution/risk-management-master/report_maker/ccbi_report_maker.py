...
if __name__ == '__main__':
	risk_metrics = pd.read_csv('ccbi/pv_ts_sg_stats.csv', index_col=0)
	risk_metrics_2018 = pd.read_csv('ccbi/pv_ts_sg_stats_2018.csv', index_col=0)
	complete_performance = pd.read_csv('ccbi/complete_performance.csv', index_col=0, parse_dates=True)
	complete_performance_2018 = pd.read_csv('ccbi/complete_performance_2018.csv', index_col=0, parse_dates=True)
	holdings = pd.read_csv('ccbi/holdings.csv')
	# rotation_holdings = pd.read_csv('bochkam/rotation_holdings.csv')
	regional_dis = pd.read_csv('ccbi/regional_dis.csv')
	duration = pd.read_csv('ccbi/duration.csv')
	underlyings = pd.read_csv('ccbi/underlyings.csv')
	monthly_return = pd.read_csv('ccbi/monthly_return.csv', index_col=0)

	env = Environment(loader=FileSystemLoader('.'))
	template = env.get_template('ccbi/template.html')
	...
	plt.savefig('ccbi/complete_performance.png', format='png')
	complete_performance_2018.plot(color=['#EE2F79', '#00ABC0'])
	plt.savefig('ccbi/complete_performance_2018.png', format='png')

	risk_metrics = risk_metrics.applymap('{:.2%}'.format)
	risk_metrics_2018 = risk_metrics_2018.applymap('{:.2%}'.format)
	# monthly_return = monthly_return.applymap('{:.2%}'.format)
	...
	template_vars = {
		...
		"underlyings": underlyings.to_html(index=False),
		"monthly_ret": monthly_return.to_html()
	}

	html_out = template.render(template_vars)
	f = open('ccbi/report.html', 'w')
	f.write(html_out)
	f.close()
	...
	url_dir = '/home/aqumon/www/chart.aqumon.com/2B-materials/CCBI/' \
			  + end.replace('-', '') + '/'
	upload(
