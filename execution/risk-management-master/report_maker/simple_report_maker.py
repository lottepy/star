...
if __name__ == '__main__':
	risk_metrics = pd.read_csv('simple/pv_ts_sg_stats.csv', index_col=0)
	risk_metrics_2018 = pd.read_csv('simple/pv_ts_sg_stats_2018.csv', index_col=0)
	complete_performance = pd.read_csv('simple/complete_performance.csv', index_col=0, parse_dates=True)
	complete_performance_2018 = pd.read_csv('simple/complete_performance_2018.csv', index_col=0, parse_dates=True)
	holdings = pd.read_csv('simple/holdings.csv')
	# rotation_holdings = pd.read_csv('bochkam/rotation_holdings.csv')
	regional_dis = pd.read_csv('simple/regional_dis.csv')
	duration = pd.read_csv('simple/duration.csv')

	env = Environment(loader=FileSystemLoader('.'))
	template = env.get_template('simple/template.html')
	...
	plt.savefig('simple/complete_performance.png', format='png')
	complete_performance_2018.plot()
	plt.savefig('simple/complete_performance_2018.png', format='png')
	...
	f = open('simple/report.html', 'w')
	f.write(html_out)
	f.close()

	# files
	localpath_dir = 'simple/'
	# url_dir = '/home/aqumon/www/chart.aqumon.com/BOCHKAM/20190107/'
	url_dir = '/home/aqumon/www/chart.aqumon.com/2B-materials/simple/' \
			  + end.replace('-', '') + '/'
	upload(
