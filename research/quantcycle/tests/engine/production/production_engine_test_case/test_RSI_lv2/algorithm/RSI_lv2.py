...
class Allocation_strategy(BaseStrategy):
    ...
    def on_data(self, data_dict: dict, time_dict: dict, ref_data_dict: dict, ref_time_dict: dict):
        ...
        get_logger.get_logger().info(f"long_metrics_name:{long_metrics_name}")
        get_logger.get_logger().info(f"time_dict:{time_dict[long_metrics_name]}")
        if len(time_dict[long_metrics_name])==1:
