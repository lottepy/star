...
class RSI_strategy(BaseStrategy):
    ...
    def on_data(self, data_dict: dict, time_dict: dict, ref_data_dict: dict, ref_time_dict: dict):
        if 'FX_MAIN' in data_dict:
            window_data = data_dict['FX_MAIN']
            time_data = time_dict['FX_MAIN']
            current_data = window_data[-1, :, 3]
            current_fx_data = data_dict["FX_CCPFXRATE"][-1, :, 0]
        else:       # 'STOCKS_MAIN' in data_dict
            window_data = data_dict['STOCKS_MAIN']
            time_data = time_dict['STOCKS_MAIN']
            current_data = window_data[-1, :, 3]
            current_fx_data = data_dict["STOCKS_CCYFX"][-1, :, 0]
        ...
        target_holding = weight * ref_aum/(current_data * current_fx_data)
