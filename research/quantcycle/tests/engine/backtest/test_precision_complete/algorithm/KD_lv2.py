...
class Allocation_strategy(BaseStrategy):
    ...
    def on_data(self, data_dict: dict, time_dict: dict, ref_data_dict: dict, ref_time_dict: dict):
        ...
        ###############################
        # KD Hardcode
        short_pnl_threshold = 0.03
        short_sharp_ratio_threshold = 1.5
        short_hit_ratio_threshold = 0
        short_profit_rate_threshold = 0.51
        short_number_of_trade_threshold = 5
        short_average_drawdown_threshold = 0.007
        short_expected_return_threshold = -9999

        long_pnl_threshold = 0.07
        long_sharp_ratio_threshold = 0.8
        long_hit_ratio_threshold = 0
        long_profit_rate_threshold = 0.46
        long_number_of_trade_threshold = 15
        long_average_drawdown_threshold = 0.007
        ...
        if long_metrics_name in time_dict:
            ...
            if time_dict[long_metrics_name][-1,0] == \
                ...
                all_strategy_short = np.concatenate((all_strategy_short, expect_return_short.reshape(-1,1),
                                                     np.arange(all_strategy_short.shape[0]).astype(np.float64).reshape(-1,1)), axis=1)
                all_strategy_long = np.concatenate((all_strategy_long, expect_return_long.reshape(-1,1),
                                                     np.arange(all_strategy_long.shape[0]).astype(np.float64).reshape(-1,1)), axis=1)
                ...
                if rank_metrics == Rank_metrics.Expected_Return.value:
                    ...
                    all_strategy_short[:, Metrics.holding_period_return.value] = np.round(all_strategy_short[:, Metrics.holding_period_return.value], 8,
                                                                                          all_strategy_short[:, Metrics.holding_period_return.value])
                    all_strategy_short = all_strategy_short[np.argsort(all_strategy_short[:,Metrics.holding_period_return.value])]
