...
class Allocation_strategy(BaseStrategy):
    ...
    def on_data(self, data_dict: dict, time_dict: dict, ref_data_dict: dict, ref_time_dict: dict):
        ...
        #eps = 1e-5
        if current_time == 1509494400 + 6*60*60 + 57*60 : # 2017年11月1日WednesdayAM12点00分 First time point
            ...
            if data_dict['main'].shape == (2, 8, 4) and time_dict['main'].shape == (2, 9) and \
                ...
                data_dict['RSI_pnl'].shape == (1, 768, 1) and time_dict['RSI_pnl'].shape == (1, 9) and \
                data_dict['RSI_position'].shape == (1, 768, 1) and time_dict['RSI_position'].shape == (1,9):
                remark['input_array_shape_w/o_ref_w/o_signal_remark_correct'] = 1
            else:
                remark['input_array_shape_w/o_ref_w/o_signal_remark_correct'] = 0
        
        elif current_time == 1515974400 + 6*60*60 + 57*60 :      # 2018-01-15 00:00:00 First time point to have signal_remark data
            # data_dict: main/fxrate/int & RSI_pnl/RSI_position
            ...
            if data_dict['main'].shape == (2, 8, 4) and time_dict['main'].shape == (2, 9) and \
               data_dict['fxrate'].shape == (1, 8, 1) and time_dict['fxrate'].shape == (1, 9) and \
               data_dict['int'].shape == (1, 8, 1) and time_dict['int'].shape == (1, 9) and \
               data_dict['RSI_pnl'].shape == (53, 768, 1) and time_dict['RSI_pnl'].shape == (53, 9) and \
               data_dict['RSI_position'].shape == (1, 768, 1) and time_dict['RSI_position'].shape == (1, 9) and \
               ref_data_dict['signal_remark'].shape == (1, 96, 5) and ref_time_dict['signal_remark'].shape == (1, 9):
                remark['input_array_shape_w/o_ref_correct'] = 1
            else:
                remark['input_array_shape_w/o_ref_correct'] = 0
        
        elif current_time == 1517356800 + 6*60*60 + 57*60 : # 2018年1月31日WednesdayAM12点00分 First time point to have 'RSI_metrics_61' in main_data
            ...
            if data_dict['main'].shape == (2, 8, 4) and time_dict['main'].shape == (2, 9) and \
               data_dict['fxrate'].shape == (1, 8, 1) and time_dict['fxrate'].shape == (1, 9) and \
               data_dict['int'].shape == (1, 8, 1) and time_dict['int'].shape == (1, 9) and \
               data_dict['RSI_pnl'].shape == (65, 768, 1) and time_dict['RSI_pnl'].shape == (65, 9) and \
               data_dict['RSI_position'].shape == (1, 768, 1) and time_dict['RSI_position'].shape == (1,9) and \
               data_dict['RSI_metrics_61'].shape == (1, 768, 15) and time_dict['RSI_metrics_61'].shape == (1,9) and \
               ref_data_dict['signal_remark'].shape == (1, 96, 5) and ref_time_dict['signal_remark'].shape == (1, 9):
                remark['input_array_shape_w/o_ref_w_metrics61_correct'] = 1
            else:
                remark['input_array_shape_w/o_ref_w_metrics61_correct'] = 0

        # ref_data_dict['ref_hourly'] starts from 1536796800  2018年9月13日ThursdayAM12点00分
        # shape = (7, 3, 1)

        elif current_time == 1536883200 + 6*60*60 + 57*60 :    # 2018-09-14 00:00:00
            # data_dict: main/fxrate/int & RSI_pnl/RSI_position/ RSI_metrics_61
            ...
            if data_dict['main'].shape == (2, 8, 4) and time_dict['main'].shape == (2, 9) and \
               data_dict['fxrate'].shape == (1, 8, 1) and time_dict['fxrate'].shape == (1, 9) and \
               data_dict['int'].shape == (1, 8, 1) and time_dict['int'].shape == (1, 9) and \
               data_dict['RSI_pnl'].shape == (217, 768, 1) and time_dict['RSI_pnl'].shape == (217, 9) and \
               data_dict['RSI_position'].shape == (1, 768, 1) and time_dict['RSI_position'].shape == (1,9) and \
               data_dict['RSI_metrics_61'].shape == (1, 768, 15) and time_dict['RSI_metrics_61'].shape == (1,9) and \
               ref_data_dict['signal_remark'].shape == (1, 96, 5) and ref_time_dict['signal_remark'].shape == (1, 9) and \
        ...
        elif current_time == 1537142400 + 6*60*60 + 57*60: # 2018年9月17日MondayAM12点00分 a random time point
            # Testing static parameters in the strategy
            assert self.strategy_name == 'Allocation_strategy'
            assert len(self.strategy_param) == 0
            assert 'main' in self.metadata
            assert self.metadata['main']['symbols'][0] == '000005.SZ'
            assert self.metadata['main']['symbols'][7] == '601398.SH'
            assert len(self.metadata['main']['symbol_types']) == 8
            assert self.metadata['main']['symbol_types'][3] == 'CN_STOCK'
            assert self.metadata['main']['fields'][0] == 'open'
            assert self.metadata['main']['fields'][3] == 'close'
            assert len(self.metadata['main']['ccy_list']) == 8
            assert self.metadata['main']['ccy_list'][2] == 'CNY'
            ...
            assert len(self.metadata['remark']['fields']) == 5
            assert '0' in self.metadata
            # assert 0 not in self.metadata
            self.metadata['0']['RSI'][0] == '000005.SZ'
            assert np.nansum(self.ccy_matrix) == 8
            remark['static_parameter_correct'] = 1

        elif current_time == 1543536000 + 6*60*60 + 57*60 : # 2018年11月30日FridayAM12点00分 first time to have 'RSI_metrics_252' in main_data
            ...
            if data_dict['main'].shape == (2, 8, 4) and time_dict['main'].shape == (2, 9) and \
                data_dict['fxrate'].shape == (1, 8, 1) and time_dict['fxrate'].shape == (1, 9) and \
                data_dict['int'].shape == (1, 8, 1) and time_dict['int'].shape == (1, 9) and \
                data_dict['RSI_pnl'].shape == (254, 768, 1) and time_dict['RSI_pnl'].shape == (254, 9) and \
                data_dict['RSI_position'].shape == (1, 768, 1) and time_dict['RSI_position'].shape == (1,9) and \
                data_dict['RSI_metrics_61'].shape == (1, 768, 15) and time_dict['RSI_metrics_61'].shape == (1,9) and \
                data_dict['RSI_metrics_252'].shape == (1, 768, 15) and time_dict['RSI_metrics_252'].shape == (1,9) and \
                ref_data_dict['signal_remark'].shape == (1, 96, 5) and ref_time_dict['signal_remark'].shape == (1, 9) and \
                ref_data_dict['ref_daily'].shape == (14, 5, 4) and ref_time_dict['ref_daily'].shape == (14, 9) and \
                ref_data_dict['ref_hourly'].shape == (14, 3, 1) and ref_time_dict['ref_hourly'].shape == (14, 9):
