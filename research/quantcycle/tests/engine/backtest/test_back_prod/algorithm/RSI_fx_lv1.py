...
class RSI_strategy(BaseStrategy):
    ...
    def on_data(self, data_dict: dict, time_dict: dict, ref_data_dict: dict, ref_time_dict: dict):
        ...
        eps = 1e-5
        if current_time == 1515974400:      # 2018-01-15 00:00:00, reference data empty
            if len(data_dict) == 4 and len(time_dict) == 4 and len(ref_data_dict) == 0 and len(ref_time_dict) == 0:
            ...
            if data_dict['main'].shape == (2, 4, 4) and time_dict['main'].shape == (2, 9) and \
                ...
                data_dict['int'].shape == (2, 4, 1) and time_dict['int'].shape == (2, 9):
                remark['input_array_shape_w/o_ref_correct'] = 1
            else:
                remark['input_array_shape_w/o_ref_correct'] = 0
            
            if time_dict['fxrate'][-1, 5] == 15 and time_dict['fxrate'][-1, 6] == 0 and \
                time_dict['int'][-1, 5] == 15 and time_dict['int'][-1, 6] == 0:
                remark["main_data_time_w/o_ref_correct"] = 1
            else:
                remark["main_data_time_w/o_ref_correct"] = 0
            
            if abs(data_dict['fxrate'][-1, 1, 0] - 0.00902446) < eps and \
                abs(data_dict['int'][-1, 0, 0] - 1.75666188e-05) < eps:
                remark['main_data_w/o_ref_correct'] = 1
            else:
                remark['main_data_w/o_ref_correct'] = 0
            
        elif current_time == 1536883200:    # 2018-09-14 00:00:00, all data should be available
            if len(data_dict) == 4 and len(time_dict) == 4 and len(ref_data_dict) == 2 and len(ref_time_dict) == 2:
                remark['input_len_w_ref_correct'] = 1
            else:
                remark['input_len_w_ref_correct'] = 0
            ...
            if abs(data_dict['fxrate'][-1, 1, 0] - 0.008933756) < eps and \
               abs(data_dict['int'][-1, 0, 0] - 1.853736e-05) < eps:
                remark['main_data_w_ref_correct'] = 1
            else:
                remark['main_data_w_ref_correct'] = 0

        elif current_time == 1537228800:
            # Testing static parameters in the strategy
            assert self.strategy_name == 'RSI_strategy'
            ...
            assert 'remark' not in self.metadata
