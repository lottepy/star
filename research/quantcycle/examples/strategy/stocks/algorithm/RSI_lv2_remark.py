...
class Allocation_strategy(BaseStrategy):
    ...
    def on_data(self, data_dict: dict, time_dict: dict, ref_data_dict: dict, ref_time_dict: dict):
        # Signal remark example
        remark = Dict.empty(types.unicode_type, nb.float64)
        if 'signal_remark' in ref_data_dict:
            col_idx = self.remark_field_names.index('years_if_feb_29_open')
            remarked_year = ref_data_dict['signal_remark'][-1, 0, col_idx]    # -1: latest remark, 0: strategy id
            if not np.isnan(remarked_year) and remarked_year != self.last_remarked_year:
                remark['leap_year_found'] = remarked_year
                self.last_remarked_year = remarked_year
        self.save_signal_remark(remark)

        # Level 2 RSI allocation
