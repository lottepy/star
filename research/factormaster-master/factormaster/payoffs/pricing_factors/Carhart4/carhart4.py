...
def classic_c4():
    ...
    # MOM
    m_rebalance_dates = _get_rebalance_dates(START_DATE, END_DATE, 'M')
    top_weights = pd.DataFrame(index=index, columns=columns)
    bot_weights = pd.DataFrame(index=index, columns=columns)
    for i in tqdm(range(12, len(m_rebalance_dates))):
        date = m_rebalance_dates[i]
        one_component = components.loc[date].values
        one_1m_rtn = rtn.loc[_offset_trading_day(m_rebalance_dates[i-12], 1): m_rebalance_dates[i-1]]
        one_1m_rtn = np.prod(one_1m_rtn.values + 1, axis=0)
        mom_upper = _get_percentile(one_1m_rtn, one_component.astype(bool), 70)
        mom_lower = _get_percentile(one_1m_rtn, one_component.astype(bool), 30)
        one_top = (one_1m_rtn >= mom_upper) * one_component
        one_top[np.isnan(one_top)] = 0
        one_top = one_top / np.sum(np.abs(one_top))
        top_weights.loc[date] = one_top
        one_bot = (one_1m_rtn <= mom_lower) * one_component
    ...
    umd = (top_rtn - bot_rtn).to_frame('UMD')
    

    payoff_df = pd.concat([riskfreerate, mkt_rtn, smb, hml, umd], axis=1)
    payoff_df.to_csv('data/payoffs/pricing_factors/Carhart4/classic_c4.csv')
    return
...
def simplified_c4():
    ...
    top_weights = pd.DataFrame(index=index, columns=columns)
    bot_weights = pd.DataFrame(index=index, columns=columns)
    for i in tqdm(range(12, len(rebalance_dates))):
        date = rebalance_dates[i]
        one_component = components.loc[date].values
        one_mkt_cap = mkt_cap.loc[date].values
        one_1m_rtn = rtn.loc[_offset_trading_day(rebalance_dates[i-12], 1): rebalance_dates[i-1]]
        one_1m_rtn = np.prod(one_1m_rtn.values + 1, axis=0)
        mom_upper = _get_percentile(one_1m_rtn, one_component.astype(bool), 90)
        mom_lower = _get_percentile(one_1m_rtn, one_component.astype(bool), 10)
        one_top = (one_1m_rtn >= mom_upper) * one_component * one_mkt_cap
        one_top[np.isnan(one_top)] = 0
        one_top = one_top / np.sum(np.abs(one_top))
        top_weights.loc[date] = one_top
        one_bot = (one_1m_rtn <= mom_lower) * one_component * one_mkt_cap
        one_bot[np.isnan(one_bot)] = 0
        one_bot = one_bot / np.sum(np.abs(one_bot))
        bot_weights.loc[date] = one_bot
    top_weights = top_weights.astype(np.float64)
    ...
    payoff_df = pd.concat([riskfreerate, mkt_rtn, smb, hml, umd], axis=1)
    payoff_df.to_csv('data/payoffs/pricing_factors/Carhart4/simplified_c4.csv')
    return
...
def weekly_c4():
    ...
    top_weights = pd.DataFrame(index=index, columns=columns)
    bot_weights = pd.DataFrame(index=index, columns=columns)
    for i in tqdm(range(52, len(rebalance_dates))):
        date = rebalance_dates[i]
        one_component = components.loc[date].values
        one_mkt_cap = mkt_cap.loc[date].values
        one_1m_rtn = rtn.loc[_offset_trading_day(rebalance_dates[i-52], 1): rebalance_dates[i-4]]
        one_1m_rtn = np.prod(one_1m_rtn.values + 1, axis=0)
        mom_upper = _get_percentile(one_1m_rtn, one_component.astype(bool), 90)
        mom_lower = _get_percentile(one_1m_rtn, one_component.astype(bool), 10)
        one_top = (one_1m_rtn >= mom_upper) * one_component * one_mkt_cap
    ...
    payoff_df = pd.concat([riskfreerate, mkt_rtn, smb, hml, umd], axis=1)
    payoff_df.to_csv('data/payoffs/pricing_factors/Carhart4/weekly_c4.csv')
    return
