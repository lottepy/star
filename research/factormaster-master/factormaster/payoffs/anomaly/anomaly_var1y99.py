...
def _get_raw_features():
    def _get_var(df):
        return - np.sort(df.values)[int(len(df)*0.01)]
    res = {}
    rtn_ = pd.read_csv('data/features/rtn.csv', index_col=0, parse_dates=True)
    var = rtn_.rolling(252, min_periods = 200).apply(_get_var)
    res['var'] = var
