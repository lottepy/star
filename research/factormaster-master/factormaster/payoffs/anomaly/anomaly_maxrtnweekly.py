...
def _get_raw_features():
    res = {}
    res['maxrtn'] = pd.read_csv('data/features/rtn.csv', index_col=0, parse_dates=True).rolling(5).max()
...
def _get_exposure(formatted_data, setting):
    ...
    maxrtn = formatted_data['maxrtn']
    maxrtn = universe_mask * maxrtn
    # handle industry neutral
    exposure = _handle_industry_neutral(maxrtn, setting[2])
    return exposure
