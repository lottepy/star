...
FACTOR_NAME = "ExcessReturn24m"
LONG_HIGH = True
SUFFIX_LIST = [i+str(j) for i,j in itertools.product(['0000', '0001', '0100', '0101'], range(3))]
...
def _get_raw_features():
    res = {}
    res['rtn'] = pd.read_csv('data/features/log_rtn.csv', index_col=0, parse_dates=True)
    tmp = pd.read_excel('data/features/AShareIndex.xlsx', index_col=0, parse_dates=True)
    market_index = {i: np.log(tmp[[i]]).diff().loc[res['rtn'].index[0]: res['rtn'].index[-1]] for i in DIGIT_MAPPING[4].values()}
    excess_rtn = {f'exrtn-{i}': (res['rtn'] - market_index[i].values).rolling(21*24).sum() for i in market_index.keys()}
    res.update(excess_rtn)
...
def _get_exposure(formatted_data, setting):
    ...
    rtn_ = formatted_data[f'exrtn-{setting[4]}']
    rtn_ = universe_mask * rtn_
    # handle industry neutral
    exposure = _handle_industry_neutral(rtn_, setting[2])
    return exposure
