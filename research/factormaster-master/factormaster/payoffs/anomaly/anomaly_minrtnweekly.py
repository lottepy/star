...
FACTOR_NAME = "MinRtnWeekly"
LONG_HIGH = True
SUFFIX_LIST = ['0100', '0101']
...
def _get_raw_features():
    res = {}
    res['minrtn'] = pd.read_csv('data/features/rtn.csv', index_col=0, parse_dates=True).rolling(5).min()
...
def _get_exposure(formatted_data, setting):
    ...
    minrtn = formatted_data['minrtn']
    minrtn = universe_mask * minrtn
    # handle industry neutral
    exposure = _handle_industry_neutral(minrtn, setting[2])
    return exposure
