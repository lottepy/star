...
def _get_raw_features():
    res = {}
    log_rtn = pd.read_csv('data/features/log_rtn.csv', index_col=0, parse_dates=True)
    riskfreerate = pd.read_csv('data/features/riskfreerate.csv', index_col=0, parse_dates=True).loc[log_rtn.index]
    log_excess_rtn = log_rtn - np.log(1+riskfreerate.values)
    res['ewrtn'] = pd.DataFrame(index=log_excess_rtn.index, columns=log_excess_rtn.columns)
    for i in tqdm(range(503, len(log_excess_rtn))):
        one_log_excess_rtn = log_excess_rtn.iloc[i-503: i+1]
        res['ewrtn'].iloc[i] = one_log_excess_rtn.ewm(halflife=126).mean().iloc[-1]
    res['ewrtn'] = res['ewrtn'].shift(21)
