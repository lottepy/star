...
def _get_raw_features():
    res = {}
    res['ROA'] = pd.read_csv('data/features/roa.csv', index_col=0, parse_dates=True)
...
def _get_exposure(formatted_data, setting):
    ...
    roa = formatted_data['ROA']
    roa = universe_mask * roa
    # handle industry neutral
    exposure = _handle_industry_neutral(roa, setting[2])
    return exposure
