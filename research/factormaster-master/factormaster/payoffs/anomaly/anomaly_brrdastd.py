...
def _get_raw_features():
    ...
    res['ewstd'] = pd.DataFrame(index=log_excess_rtn.index, columns=log_excess_rtn.columns)
    for i in tqdm(range(251, len(log_excess_rtn))):
        one_log_excess_rtn = log_excess_rtn.iloc[i-251: i+1]
        res['ewstd'].iloc[i] = one_log_excess_rtn.ewm(halflife=42).std().iloc[-1]
...
def _get_exposure(formatted_data, setting):
    ...
    exposure = formatted_data[f'ewstd']
