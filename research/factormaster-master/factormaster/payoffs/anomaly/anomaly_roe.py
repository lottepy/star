...
def _get_raw_features():
    res = {}
    res['ROE'] = pd.read_csv('data/features/roe.csv', index_col=0, parse_dates=True)
...
def _get_exposure(formatted_data, setting):
    ...
    roe = formatted_data['ROE']
    roe = universe_mask * roe
    # handle industry neutral
    exposure = _handle_industry_neutral(roe, setting[2])
    return exposure
