...
def _get_raw_features():
    res = {}
    amount = pd.read_csv('data/features/amount.csv', index_col=0, parse_dates=True)
    rtn = pd.read_csv('data/features/rtn.csv', index_col=0, parse_dates=True)
    illiq = rtn.abs()/amount
    res['illiq'] = np.log(illiq.rolling(5).mean())
