...
def _get_raw_features():
    res = {}
    lookback_days = 10
    tmp = pd.read_csv('data/features/log_rtn.csv', index_col=0, parse_dates=True)
    res['rtn'] = tmp.rolling(lookback_days).sum() / np.sqrt((tmp**2).rolling(lookback_days).sum())
