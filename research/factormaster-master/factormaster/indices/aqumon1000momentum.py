...
INDEX_NAME = 'AQUMON1000_MOMENTUM'
...
def _get_factors():
    ...
    size_neutral = True
    mom = pd.read_csv('data/features/log_rtn.csv', index_col=0, parse_dates=True)
    mom = mom.rolling(252-21).sum().shift(21)
    mom = mom.loc[SSE_CALENDAR.loc[START_DATE: END_DATE].index, STOCK_LIST]
    value = mom * (UNIVERSE.replace({0: np.nan})).astype(float)
    # --------------------------
