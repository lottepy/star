...
def _get_factors():
    ...
    size_neutral = True
    exposure = pd.read_csv('data/features/roayoy.csv', index_col=0, parse_dates=True)
    ...
    industry_neutral = True
    size_neutral = True
    exposure = pd.read_csv('data/features/roeyoy.csv', index_col=0, parse_dates=True)
    exposure = exposure.loc[SSE_CALENDAR.loc[START_DATE: END_DATE].index, STOCK_LIST]
    value = exposure * (UNIVERSE.replace({0: np.nan}))
    # --------------------------
    value = value.astype(float)
    ...
    industry_neutral = True
    size_neutral = True
    exposure = pd.read_csv('data/features/niyoy.csv', index_col=0, parse_dates=True)
    exposure = exposure.loc[SSE_CALENDAR.loc[START_DATE: END_DATE].index, STOCK_LIST]
    value = exposure * (UNIVERSE.replace({0: np.nan}))
    # --------------------------
    value = value.astype(float)
    ...
    industry_neutral = True
    size_neutral = True
    exposure = pd.read_csv('data/features/oryoy.csv', index_col=0, parse_dates=True)
    exposure = exposure.loc[SSE_CALENDAR.loc[START_DATE: END_DATE].index, STOCK_LIST]
    value = exposure * (UNIVERSE.replace({0: np.nan}))
    # --------------------------
    value = value.astype(float)
