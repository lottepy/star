...
def _get_factors():
    ...
    size_neutral = True
    exposure = pd.read_csv('data/features/bm.csv', index_col=0, parse_dates=True)
    ...
    industry_neutral = True
    size_neutral = True
    exposure = pd.read_csv('data/features/ep.csv', index_col=0, parse_dates=True)
    exposure = exposure.loc[SSE_CALENDAR.loc[START_DATE: END_DATE].index, STOCK_LIST]
    value = exposure * (UNIVERSE.replace({0: np.nan}))
    # --------------------------
    value = value.astype(float)
    ...
    industry_neutral = True
    size_neutral = True
    exposure = pd.read_csv('data/features/ebit2ev.csv', index_col=0, parse_dates=True)
    exposure = exposure.loc[SSE_CALENDAR.loc[START_DATE: END_DATE].index, STOCK_LIST]
    value = exposure * (UNIVERSE.replace({0: np.nan}))
    # --------------------------
    value = value.astype(float)
    ...
    industry_neutral = True
    size_neutral = True
    exposure = pd.read_csv('data/features/gp2ev.csv', index_col=0, parse_dates=True)
    exposure = exposure.loc[SSE_CALENDAR.loc[START_DATE: END_DATE].index, STOCK_LIST]
    value = exposure * (UNIVERSE.replace({0: np.nan}))
    # --------------------------
    value = value.astype(float)
