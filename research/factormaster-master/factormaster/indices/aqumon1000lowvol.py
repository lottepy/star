...
def _get_factors():
    ...
    size_neutral = True
    vol = pd.read_csv('data/features/idio_resid.csv', index_col=0, parse_dates=True)
    vol = vol.ewm(halflife=63).std()
    vol = vol.loc[SSE_CALENDAR.loc[START_DATE: END_DATE].index, STOCK_LIST]
    value = vol * (UNIVERSE.replace({0: np.nan}))
    # --------------------------
