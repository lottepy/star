...
# files = os.listdir()
# path_USD = str(os.getcwd()) + '/JP_ac/data/*.csv'
...
all_data = pd.read_csv('China_ac.csv', index_col=0,  parse_dates=True)
date_rng = pd.date_range(start=all_data.index[0], end=all_data.index[-1], freq='D')

all_data = pd.DataFrame(data= all_data, index= date_rng).fillna(method='ffill')
all_data.index = [x.isoformat() for x in all_data.index.to_datetime().date]
...
# 保存文件
final_result.to_csv('China_ac_return.csv', encoding='utf-8')
