...
from tia.util import exceptions
...
class Request(object):
    ...
    def raise_exception(self):
        if not self.ignore_security_error and len(self.security_errors) > 0:
            ...
            raise exceptions.BBGSecurityError('SecurityError: %s' % ','.join(msgs))
        if not self.ignore_field_error and len(self.field_errors) > 0:
            msgs = ['(%s, %s, %s, %s)' % (s.security, s.field, s.category, s.message) for s in self.field_errors]
            raise exceptions.BBGFieldError('FieldError: %s' % ','.join(msgs))
        raise exceptions.BBGException('Programmer Error: No exception to raise')
...
class Terminal(object):
    ...
    def execute(self, request):
        ...
        if not session.start():
            raise exceptions.BBGSessionStartError('failed to start session')

        try:
            self.logger.info('executing request: %s' % repr(request))
            if not session.openService(request.svcname):
                raise exceptions.BBGServiceOpenError('failed to open service %s' % request.svcname)
...
class SyncSubscription(object):
    ...
    def _init(self):
        ...
        if not session.openService('//blp/mktdata'):
            raise exceptions.BBGServiceOpenError('failed to open service')
    ...
    def on_subscription_status(self, evt):
        for msg in XmlHelper.message_iter(evt):
            if msg.messageType() == 'SubscriptionFailure':
                ...
                raise exceptions.BBGSubscriptionError('subscription failed sid=%s desc=%s' % (sid, desc))
